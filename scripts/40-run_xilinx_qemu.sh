...
QEMU_TARGET="${RUN_DIR}/qemu/qemu_install/bin/qemu-system-x86_64"
IMG_SIZE="10G"
VM_MEM_SIZE="16G"
VM_SMP_NUM="8"
RP_PCIE_SLOT_NUM="0"
...
BIOS_PATH="${RUN_DIR}/qemu/qemu_install/share/qemu/bios-256k.bin"
...
if [ "${ENABLE_CPM_SIMULATION}" -eq 0 ]; then
    QEMU_COMMAND+=""

    eval "sudo ${QEMU_COMMAND} &>${QEMU_LOG_PATH} & disown;"
fi

if [ "${ENABLE_CPM_SIMULATION}" -eq 1 ]; then
    QEMU_COMMAND+=" -device ioh3420,id=rootport1,slot=${PCIE_ROOT_SLOT_NUM} -device remote-port-pci-adaptor,bus=rootport1,id=rp0 -device remote-port-pcie-root-port,id=rprootport,slot=${RP_PCIE_SLOT_NUM},rp-adaptor0=rp,rp-chan0=${RP_CHAN_NUM} -machine-path ${RUN_DIR}"
